...
Nearly every step here is the mirror image of a check in `checks.py`: the
export is what the editor can express, the published graph is what a consumer
should receive. Running `check` over the output of `publish` should come back
clean, which is the build's real test. The pairing is not quite exact -- some
checks report a defect only the source can fix, and step 6 fixes more than its
check reports -- and where it breaks, the step says so.
...
#: Everything above plus the scheme's own description, which is prose a reader
#: sees too. Mirrors `checks.PROSE`.
...
#: Two or more spaces between non-space characters, exactly as `padded-literal`
#: reports them. Bounded that way so indentation on a continuation line of a
#: bulleted scope note survives.
INNER_RUN = re.compile(r"(?<=\S)[ \t]{2,}(?=\S)")
#: Horizontal whitespace at the end of a line, including the last one.
TRAILING_RUN = re.compile(r"[ \t]+$", re.MULTILINE)
...
def publish(vocab: Vocabulary) -> Graph:
    ...
    # 6. Invisible whitespace. A double space cannot be seen while typing it, so
    #    it accumulates, and HTML collapses it on the way out -- which is why a
    #    page reads correctly while the RDF underneath says `with  JavaScript`.
    #    Newlines are load-bearing (TimeCluster's scope note is a bulleted list)
    #    so only horizontal whitespace is touched. Deliberately broader than its
    #    check: `padded-literal` reports the mid-prose runs, and the leading,
    #    trailing and end-of-line padding fixed here goes unreported, because
    #    reporting it would add invisible chores no reader can perceive.
    #    Runs before step 7 -- DOUBLED_ATTRIBUTION matches `) (by` with a single
    #    space, so collapsing first makes that step strictly more effective.
    for prop in PROSE:
        for s, o in list(out.subject_objects(prop)):
            if not isinstance(o, Literal) or o.datatype is not None:
                continue
            fixed = TRAILING_RUN.sub("", INNER_RUN.sub(" ", str(o))).strip()
            if fixed != str(o):
                out.remove((s, prop, o))
                out.add((s, prop, Literal(fixed, lang=o.language)))

    # 7. Change notes are generated by the editor, and it leaks its own
    ...
    # 8. Mapping properties have domain and range skos:Concept, so pointing one
    ...
    # 9. Collections come out of the editor with rdfs:label and skos:member and
    ...
    # 10. A versionIRI names this snapshot, so a consumer can cite what they read.
