...
The two checks that read prose blank quoted spans before reading them, which is what makes an error-level severity safe here. The editor's change notes quote the string they record fixing — `Corrected typo from "definiton" to "definition"` — and fourteen of the fifteen misspellings in this vocabulary are that pattern, spelled wrong on purpose and correctly so.
